"""
公司可视化数据处理器（重构版）
基于 BaseVisualizationProcessor，专门处理公司研报的可视化需求
"""
...
class CompanyVisualizationDataProcessor(BaseVisualizationProcessor):
    """公司可视化数据处理器，将可视化建议与实际数据结合，生成高质量的公司分析图表"""
    ...
    def __init__(
        ...
        self._assets_output_dir = (
            ...
            os.path.join(project_root, "test_company_datas", "images")
        )
    ...
    def get_chart_query_context(
        ...
        if "由于数据量较大" in data_content:
            # 分块处理的情况
            chart_query = f"""目标: 为卖方研究报告第{section}章节生成{chart_type}图表，展示"{chart_title}"。
...
注意：由于数据量较大，以下是关键数据块的概述。请仔细提取其中的数字信息和时间序列数据生成准确的专业图表：

{data_content}

重要提示：
1. 优先关注数字数据、财务指标、时间序列信息
2. 确保图表数据的准确性和逻辑性
3. 生成符合卖方研报标准的静态图表
4. 配色专业，服务于{report_value}的分析目的
5. 标签完整可见，无需鼠标悬浮即可阅读所有信息"""
        else:
            # 完整数据处理的情况
            chart_query = f"""目标: 为卖方研究报告第{section}章节生成{chart_type}图表，展示"{chart_title}"。
...
5. 无需鼠标悬浮即可阅读所有信息"""
    ...
    def get_incremental_enhancement_system_prompt(self) -> str:
        """获取增量增强的系统提示词"""
        return """你是一个专业的公司分析专家，负责对公司研报图表数据进行增量增强。
...
增量增强要求：
1. 保持公司分析的专业性和投资价值导向
2. 基于实际数据内容提取关键财务指标、业务数据和市场表现
3. 识别数据中的投资亮点、业绩趋势和竞争优势
4. 整合新数据与之前分析的结果
5. 提供丰富的细节信息，包括具体数值、同比增长、行业排名等
6. 保留所有重要的定量和定性信息

输出要求：
- 返回增强后的完整数据分析内容
- 包含具体的财务数据、业务指标、市场地位分析
- 保持投资研究的专业深度和准确性
...
请进行初步的公司数据分析，提取关键信息、财务指标和业务数据。这是多段数据处理的第一段，请为后续数据整合做好基础。

要求：
1. 提取所有相关的具体财务数据和业务指标
2. 识别公司业绩趋势、竞争地位、发展动态
3. 分析数据的投资价值和图表展示意义
4. 为后续数据整合准备分析框架"""
