...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="生成宏观研究报告",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  python run_macro_research_report.py --macro_name "生成式AI基建与算力投资趋势" --time "2023-2026"
  python run_macro_research_report.py --macro_name "人工智能+政策效果评估" --time "2023-2025"
  python run_macro_research_report.py --macro_name "人工智能+政策效果评估" --time "2023-2025" --to_docx
  python run_macro_research_report.py --macro_name "人工智能+政策效果评估" --time "2023-2025" --to_docx --docx_output "reports/ai_policy.docx"
  uv run run_macro_research_report.py --macro_name "生成式AI基建与算力投资趋势" --time "2023-2026"
        """)
    
    parser.add_argument('--macro_name', required=True, help='宏观主题名称')
    parser.add_argument('--time', default='2023-2025', help='时间范围 (默认: 2023-2025)')
    ...
    # 构建调用参数
    cmd = [
        sys.executable, 'generate_report.py', 'macro',
            ...
            if args.output:
                ...
                if args.data_dir:
                    ...
                    if os.path.exists(generic_data_dir):
                        ...
                        # 如果通用目录不存在，使用特定命名规则
                        safe_name = args.macro_name.replace(' ', '_').replace('/', '_').replace('&', 'and')
                        data_directory = f"test_macro_datas_{safe_name}"
                md_file = os.path.join(data_directory, f"{args.macro_name}_{args.time}_research_report.md")
