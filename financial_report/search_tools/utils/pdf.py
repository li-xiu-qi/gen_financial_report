...
def parse_pdf(filename: str, target: str = None) -> dict | None:
    serv = ApiClient(base_url=getenv('PDF_BASE_URL'))
    if serv.health_check():
        res = serv.upload_pdf(pdf_path=Path(filename))
        if res and target:
...
if __name__ == "__main__":
    client = ApiClient(base_url="http://localhost:10001")
    if client.health_check():
        result = client.upload_pdf(pdf_path=Path(
            '/home/xiaoke/projects/financial_tasks/designs/company_report/report/Nodes/.cache/.getSummary.cache/202504241815051768869610_tc.pdf'))
        print(result)
